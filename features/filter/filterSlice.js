...
export const filterSlice = createSlice({
    ...
    reducers: {
        ...
        addJobType: (state, { payload }) => {
            // console.log(payload);
            const isExist = state.jobList.job_type.includes(payload);
            // console.log(isExist);
            if (!isExist) {
                state.jobList.job_type.push(payload);
            } else {
                state.jobList.job_type = state.jobList.job_type.filter(
                    (item) => item !== payload
                );
            }
        },
        clearJobType: (state) => {
            state.jobList.job_type = [];
        },
